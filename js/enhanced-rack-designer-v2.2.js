...
class EnhancedRackDesigner {
    ...
    createCustomDragPreview() {
        ...
        if (this.draggedDevice.isInfrastructure) {
            ...
            // Drag preview should match this EXACT visual size (no additional scaling)
            const visualWidth = rawWidth * 0.7;  // Match canvas transform scale
            const visualHeight = rawHeight * 0.7; // Match canvas transform scale
            ...
            // Set drag preview to exact visual size (no zoom multiplication)
            svg.setAttribute('width', visualWidth + 'px');
            svg.setAttribute('height', visualHeight + 'px');
            // ViewBox should match raw dimensions so content renders correctly
            svg.setAttribute('viewBox', `${isEnclosed ? -50 : -15} -10 ${rawWidth} ${rawHeight}`);
