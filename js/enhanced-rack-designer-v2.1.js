...
class EnhancedRackDesigner {
    ...
    setupEventListeners() {
        ...
        // Add coordinate testing buttons
        this.addCoordinateTestButtons();
        
        // Initial render
        this.render();
    }
    
    addCoordinateTestButtons() {
        // Add test buttons to help debug coordinate issues
        const sidebar = document.querySelector('.sidebar');
        if (!sidebar) return;
        ...
        testSection.innerHTML = `
            <h4>Coordinate Debug Tools</h4>
            <button id="enableClickTest" style="margin: 2px; padding: 5px;">Enable Click Test</button>
            <button id="clearMarkers" style="margin: 2px; padding: 5px;">Clear Markers</button>
            <div id="clickResults" style="margin-top: 10px; max-height: 200px; overflow-y: auto;"></div>
        `;
        
        document.body.appendChild(testSection);
        
        // Enable click testing
        document.getElementById('enableClickTest').addEventListener('click', () => {
            this.enableClickTesting();
        });
    ...
    enableClickTesting() {
        ...
        // Temporarily disable drag events and enable click testing
        const originalHandler = svg.onclick;
        ...
        // Change button text
        document.getElementById('enableClickTest').textContent = 'Click Test Active';
        document.getElementById('enableClickTest').style.background = '#90EE90';
        
        // Add instructions
        const results = document.getElementById('clickResults');
        results.innerHTML = '<strong>Click anywhere on the canvas to test coordinates!</strong><br>';
    }
    ...
    handleTestClick(e) {
        ...
        // Check what transform is actually applied to mainCanvasGroup
        const mainGroup = document.getElementById('mainCanvasGroup');
        const actualTransform = mainGroup ? mainGroup.getAttribute('transform') : 'none';
        
        // For now, let's try making world coordinates = SVG coordinates to see if blue marker aligns
        const worldX = svgX;  // NO TRANSFORM - test if this makes blue marker align with red
        const worldY = svgY;
        
        console.log('Actual transform on mainCanvasGroup:', actualTransform);
        console.log('Our pan/zoom values:', this.pan.x, this.pan.y, this.zoom);
        
        // Place a test marker
        this.placeTestMarker(worldX, worldY, svgX, svgY);
        ...
        results.innerHTML += `
            <div style="border-bottom: 1px solid #ccc; padding: 5px; margin: 2px 0;">
                <strong>${timestamp}</strong><br>
                Raw Mouse: (${clientX}, ${clientY})<br>
                SVG Rect: left=${rect.left}, top=${rect.top}, width=${rect.width}, height=${rect.height}<br>
                SVG Coords: (${svgX}, ${svgY})<br>
                Pan: (${this.pan.x}, ${this.pan.y})<br>
                Zoom: ${this.zoom}<br>
                World Coords: (${Math.round(worldX)}, ${Math.round(worldY)})<br>
                <strong style="color: red;">RED marker offset = SVG calculation wrong!</strong>
            </div>
        `;
        
        // ADDITIONAL DEBUG: Let's check what element we're actually getting the rect from
        console.log('SVG element:', svg);
        console.log('SVG element ID:', svg.id);
        console.log('SVG getBoundingClientRect():', rect);
        console.log('SVG offsetLeft:', svg.offsetLeft, 'offsetTop:', svg.offsetTop);
        console.log('SVG clientLeft:', svg.clientLeft, 'clientTop:', svg.clientTop);
        
        // Check parent elements that might affect positioning
        let element = svg;
        let level = 0;
        while (element && level < 5) {
            console.log(`Element level ${level}:`, element.tagName, element.className, element.getBoundingClientRect());
    ...
    placeTestMarker(worldX, worldY, svgX, svgY) {
        const svg = document.getElementById('rackDesignSVG');
        if (!svg) return;
        
        // Create marker group
        const markerGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        markerGroup.setAttribute('class', 'test-marker');
        
        // Blue marker in world coordinates (goes through transform) - LARGEST
        const worldMarker = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        worldMarker.setAttribute('cx', worldX);
        worldMarker.setAttribute('cy', worldY);
        worldMarker.setAttribute('r', '20');
        worldMarker.setAttribute('fill', '#0000FF');
        worldMarker.setAttribute('stroke', '#FFFFFF');
        worldMarker.setAttribute('stroke-width', '3');
        
        // Add to main canvas group (will be transformed)
        const mainGroup = document.getElementById('mainCanvasGroup');
        if (mainGroup) {
            mainGroup.appendChild(worldMarker);
        }
        
        // Red marker in direct SVG coordinates (bypasses transform) - MEDIUM
        const directMarker = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        directMarker.setAttribute('cx', svgX);
        directMarker.setAttribute('cy', svgY);
        directMarker.setAttribute('r', '12');
        directMarker.setAttribute('fill', '#FF0000');
        directMarker.setAttribute('stroke', '#FFFFFF');
        directMarker.setAttribute('stroke-width', '2');
        directMarker.setAttribute('class', 'direct-marker');
        
        // Add directly to SVG (no transform)
        svg.appendChild(directMarker);
        
        // YELLOW MARKER: Use absolute HTML positioning to verify screen coordinates
        const htmlMarker = document.createElement('div');
        htmlMarker.innerHTML = 'YELLOW';
        htmlMarker.style.position = 'fixed';
        htmlMarker.style.left = (clientX - 25) + 'px';
        htmlMarker.style.top = (clientY - 25) + 'px';
        htmlMarker.style.width = '50px';
        htmlMarker.style.height = '50px';
        htmlMarker.style.background = 'yellow';
        htmlMarker.style.border = '3px solid black';
        htmlMarker.style.borderRadius = '50%';
        htmlMarker.style.pointerEvents = 'none';
        htmlMarker.style.zIndex = '999999';
        htmlMarker.style.fontSize = '10px';
        htmlMarker.style.textAlign = 'center';
        htmlMarker.style.lineHeight = '44px';
        htmlMarker.style.fontWeight = 'bold';
        htmlMarker.className = 'html-test-marker';
        
        console.log('Creating YELLOW marker element at coords:', clientX, clientY);
        console.log('Appending to body, current body children:', document.body.children.length);
        
        try {
            document.body.appendChild(htmlMarker);
            console.log('Successfully appended YELLOW marker');
        } catch (error) {
            console.error('Error appending YELLOW marker:', error);
        }
        
        console.log('After append, body children:', document.body.children.length);
        console.log('YELLOW marker should be at screen coords:', clientX, clientY);
        
        // Also try creating a simple alert to confirm JavaScript is working
        if (Math.random() < 0.3) {
            console.log('ALERT TEST: If you see this in console, JS is working');
        }
        
        console.log('Red marker (SVG coords):', svgX, svgY);
        console.log('Yellow marker (HTML absolute):', clientX, clientY);
        
        console.log('Placed markers: RED (direct) at', svgX, svgY, 'BLUE (world) at', worldX, worldY);
    }
    
    clearTestMarkers() {
        // Clear world coordinate markers
        const worldMarkers = document.querySelectorAll('.test-marker');
        worldMarkers.forEach(marker => marker.remove());
        
        // Clear direct coordinate markers  
        const directMarkers = document.querySelectorAll('.direct-marker');
        directMarkers.forEach(marker => marker.remove());
        
        // Clear HTML markers
        const htmlMarkers = document.querySelectorAll('.html-test-marker');
        ...
        // Reset button
        document.getElementById('enableClickTest').textContent = 'Enable Click Test';
        document.getElementById('enableClickTest').style.background = '';
        
        console.log('Cleared all test markers');
    }
    ...
    createCustomDragPreview() {
        ...
        if (this.draggedDevice.isInfrastructure) {
            ...
            // Size the SVG properly based on rack type
            const rackWidth = this.RACK_WIDTH; // 240
            const width = isEnclosed ? rackWidth + 100 : rackWidth + 30;
            const height = rackHeight * this.UNIT_HEIGHT + 40;
            
            svg.setAttribute('width', (width * this.zoom) + 'px');
            svg.setAttribute('height', (height * this.zoom) + 'px');
            svg.setAttribute('viewBox', `${isEnclosed ? -50 : -15} -10 ${width} ${height}`);
            ...
            const width = 259; // Device width from renderDevice
            const height = (this.draggedDevice.height || 1) * 24;
    ...
    handleDragOver(e) {
        ...
        // SOLUTION: The crosshair gets rendered inside mainCanvasGroup which has transform applied
        // Current transform: translate(50, 50) scale(0.7) - verified from console  
        // To make crosshair appear at cursor, we need inverse transform:
        // For transform translate(tx, ty) scale(s), inverse is: (coord - tx) / s
        const worldX = (svgX - 50) / 0.7;
        const worldY = (svgY - 50) / 0.7;
        
        // Apply inverse transform to get world coordinates 
        // SVG transform is: translate(pan.x, pan.y) scale(zoom)
        // So inverse is: scale(1/zoom) translate(-pan.x/zoom, -pan.y/zoom)
        // const worldX = svgX / this.zoom - this.pan.x / this.zoom;
        // const worldY = svgY / this.zoom - this.pan.y / this.zoom;
        ...
        if (Math.random() < 0.05) { // Occasionally
            ...
            console.log('When transformed back: x =', worldX * this.zoom + this.pan.x, 'y =', worldY * this.zoom + this.pan.y);
            console.log('Should equal cursor position:', svgX, svgY);
            console.log('Difference:', 
                'x:', (worldX * this.zoom + this.pan.x) - svgX, 
                'y:', (worldY * this.zoom + this.pan.y) - svgY);
        }
    ...
    handleDrop(e) {
        ...
        // Calculate final drop position (applies floor constraints for 42U racks)
        const rawX = (e.clientX - e.currentTarget.getBoundingClientRect().left - this.pan.x) / this.zoom;
        const rawY = (e.clientY - e.currentTarget.getBoundingClientRect().top - this.pan.y) / this.zoom;
        const finalPosition = this.calculateSnappedPosition(rawX, rawY, deviceData, false);
        const worldX = finalPosition.x;
        const worldY = finalPosition.y;

        console.log(`Drop position (snapped): ${worldX}, ${worldY}`);
    ...
    addInfrastructure(type, x, y, svgY = null) {
        ...
        if (type.startsWith('open-rack') || type.startsWith('enclosed-rack')) {
            ...
            let newY = this.FLOOR_Y - (rackHeight * this.UNIT_HEIGHT); // Position rack on floor
        ...
        if (this.draggedDevice.isInfrastructure) {
            if (this.draggedDevice.type.startsWith('open-rack') || this.draggedDevice.type.startsWith('enclosed-rack')) {
                ...
                if (isEnclosed) {
                    ...
                    // For open racks, center on the rack width
